...
void new_acc() {
    ...
    rewind(ptr);
    while (fscanf(ptr, "%d %[^|]| %d %d/%d/%d %[^|]| %[^|]| %[^|]| %[^|]| %f %d/%d/%d\n", 
           &bank.acc_no, bank.name, &bank.age, &bank.dob.month, &bank.dob.day, &bank.dob.year,
           bank.address, bank.aadhar, bank.phone, bank.acc_type, &bank.balance, 
           &bank.deposit_date.month, &bank.deposit_date.day, &bank.deposit_date.year) != EOF) {
    ...
    scanf("%d/%d/%d", &bank.deposit_date.month, &bank.deposit_date.day, &bank.deposit_date.year);
    ...
    fprintf(ptr, "%d %s| %d %d/%d/%d %s| %s| %s| %s| %.2f %d/%d/%d\n", 
            bank.acc_no, bank.name, bank.age, bank.dob.month, bank.dob.day, bank.dob.year,
            bank.address, bank.aadhar, bank.phone, bank.acc_type, bank.balance, 
            bank.deposit_date.month, bank.deposit_date.day, bank.deposit_date.year);
...
void view_list() {
    ...
    while (fscanf(view, "%d %[^|]| %d %d/%d/%d %[^|]| %[^|]| %[^|]| %[^|]| %f %d/%d/%d\n", 
           &bank.acc_no, bank.name, &bank.age, &bank.dob.month, &bank.dob.day, &bank.dob.year,
           bank.address, bank.aadhar, bank.phone, bank.acc_type, &bank.balance, 
           &bank.deposit_date.month, &bank.deposit_date.day, &bank.deposit_date.year) != EOF) {
        printf("%-10d %-20.20s %-15s $%-12.2f\n", bank.acc_no, bank.name, bank.phone, bank.balance);
        found++;
    }
...
void edit() {
    ...
    while (fscanf(old, "%d %[^|]| %d %d/%d/%d %[^|]| %[^|]| %[^|]| %[^|]| %f %d/%d/%d\n", 
           &bank.acc_no, bank.name, &bank.age, &bank.dob.month, &bank.dob.day, &bank.dob.year,
           bank.address, bank.aadhar, bank.phone, bank.acc_type, &bank.balance, 
           &bank.deposit_date.month, &bank.deposit_date.day, &bank.deposit_date.year) != EOF) {
        if (bank.acc_no == upd.acc_no) {
            ...
            if (choice == 1) {
                ...
                fprintf(newrec, "%d %s| %d %d/%d/%d %s| %s| %s| %s| %.2f %d/%d/%d\n", 
                        bank.acc_no, bank.name, bank.age, bank.dob.month, bank.dob.day, bank.dob.year,
                        upd.address, bank.aadhar, bank.phone, bank.acc_type, bank.balance, 
                        bank.deposit_date.month, bank.deposit_date.day, bank.deposit_date.year);
            } else if (choice == 2) {
                ...
                fprintf(newrec, "%d %s| %d %d/%d/%d %s| %s| %s| %s| %.2f %d/%d/%d\n", 
                        bank.acc_no, bank.name, bank.age, bank.dob.month, bank.dob.day, bank.dob.year,
                        bank.address, bank.aadhar, upd.phone, bank.acc_type, bank.balance, 
                        bank.deposit_date.month, bank.deposit_date.day, bank.deposit_date.year);
            }
...
void transact() {
    ...
    while (fscanf(old, "%d %[^|]| %d %d/%d/%d %[^|]| %[^|]| %[^|]| %[^|]| %f %d/%d/%d\n", 
        ...
        if (bank.acc_no == transaction.acc_no) {
            test = 1;
            if (stricmp(bank.acc_type, "fixed1") == 0 || stricmp(bank.acc_type, "fixed2") == 0 || stricmp(bank.acc_type, "fixed3") == 0) {
...
void see() {
    ...
    while (fscanf(ptr, "%d %[^|]| %d %d/%d/%d %[^|]| %[^|]| %[^|]| %[^|]| %f %d/%d/%d\n", 
           ...
           bank.address, bank.aadhar, bank.phone, bank.acc_type, &bank.balance, 
           &bank.deposit_date.month, &bank.deposit_date.day, &bank.deposit_date.year) != EOF) {
        if ((choice == 1 && bank.acc_no == check.acc_no) || (choice == 2 && stricmp(bank.name, check.name) == 0)) {
            test = 1;
            ...
            float interest = 0;
            if (stricmp(bank.acc_type, "saving") == 0) interest = calculate_interest(1.0/12.0, bank.balance, 8);
            else if (stricmp(bank.acc_type, "fixed1") == 0) interest = calculate_interest(1.0, bank.balance, 9);
            else if (stricmp(bank.acc_type, "fixed2") == 0) interest = calculate_interest(2.0, bank.balance, 11);
            else if (stricmp(bank.acc_type, "fixed3") == 0) interest = calculate_interest(3.0, bank.balance, 13);
            
            if (interest > 0) printf("\n\tExpected Interest: $%.2f", interest);
